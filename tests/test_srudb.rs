...
use ese_rs::{Database, EseError};

#[test]
fn test_srudb_opens() -> Result<(), EseError> {
    let db = Database::open("SRUDB.dat")?;
    assert!(db.tables().len() > 0, "Database should have tables");
    Ok(())
}
...
fn test_srudb_has_expected_structure() -> Result<(), EseError> {
    ...
    let table_names: Vec<String> = db.tables()
        .keys()
    ...
    assert!(table_names.len() >= 5, "SRUDB should have at least 5 tables");
...
fn test_srudb_column_types() -> Result<(), EseError> {
    ...
    println!("Found {} different column types in SRUDB.dat:", column_types_found.len());
